...
<body class="bg-gray-200 flex items-center justify-center min-h-screen p-4">
    <div class="bg-white w-full sm:max-w-sm md:max-w-md lg:max-w-lg xl:max-w-xl rounded-xl shadow-lg p-6 flex flex-col gap-4">
        ...
        <form action="/laporan/{{$laporan->id}}/update" method="post" autocomplete="off" class="grid grid-cols-1 gap-4 sm:grid-cols-2">
            @csrf
            ...
            <div class="flex flex-col gap-2 sm:col-span-2">
                <label for="status" class="text-sm font-medium text-gray-700">Email</label>
                <select id="" name="id_user" class="border rounded-lg p-2 focus:ring-2 focus:ring-blue-400">
                    @foreach($user as $u)
            ...
            <div class="flex flex-col gap-2 sm:col-span-2">
                ...
                name="date" 
                class="border rounded-lg p-2 focus:ring-2 focus:ring-blue-400" 
                value="{{ old('date', $laporan->date) }}" required>
            </div>
